...
if(isServer) then {
	private _parameterCorrect = params [["_noGroupToSpawn",-1,[0]]];
	private _noCachedGroups = count Spec_cache_var_cachedGroups;

	if(_parameterCorrect) then {
		private _noGroupToSpawnRounded = round _noGroupToSpawn;
		if(_noGroupToSpawn == _noGroupToSpawnRounded) then {
			if(_noGroupToSpawn < _noCachedGroups && _noGroupToSpawn >= 0) then {
				private _cachedGroupArray = Spec_cache_var_cachedGroups select _noGroupToSpawn;
				private _sizeCachedGroupArray = count _cachedGroupArray;
				if(_sizeCachedGroupArray != 0) then {
					if( _sizeCachedGroupArray == 5) then {
						_returnValue = [(_cachedGroupArray select 0),( _cachedGroupArray select 1), (_cachedGroupArray select 2), (_cachedGroupArray select 3), (_cachedGroupArray select 4)] call Spec_cache_fnc_spawnGroup_data;
						Spec_cache_var_cachedGroups set [_noGroupToSpawn,[]];
					} else {
